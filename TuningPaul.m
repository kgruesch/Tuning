...
filename = 'C:\Users\test\Documents\UpRev_Log6-24-2017_11-01-10_AM.csv'; %copy log file path here
delimiter = ',';
startRow = 2;
...
MASAFB2V1 = dataArray{:, 14};
TARGETAFR1 = dataArray{:, 15};
...
B1CORR = zeros(n,1);
B2CORR = zeros(n,1);
%% generate corrections matrix for bank 1
for i=1:n;
    %calculate average correction factor
    if TARGETAFR1(i)==30;   %ignore correction if throttle is closed
        AVGCORR1 = 1;
    else
    AVGCORR1 = (AFCORRB3/100);
    end
    if AVGCORR1 > (1+MAXCORR);  %limit correction to 15%
        AVGCORR1 = (1+MAXCORR);
    else
        AVGCORR1 = AVGCORR1;
    end
   ...
   %Calculate average AFR correction from target AFR vs. WBo2
        ...
        AFRCORR1 = AFRWBB3(i)/TARGETAFR1(i);
        end
    if AFRCORR1 > (1+MAXCORR);
        AFRCORR1 = (1+MAXCORR);
    else
        AFRCORR1 = AFRCORR1;
    end
...
%% Generate corrections matrix for bank 2
for i=1:n;
    %calculate average correction factor
    if TARGETAFR1(i)==30;   %ignore correction if throttle is closed
        AVGCORR2 = 1;
    else
    AVGCORR2 = AFCORRB4(i)/100;
    end
    if AVGCORR2 > (1+MAXCORR);  %limit correction to 15%
        AVGCORR2 = (1+MAXCORR);
    else
        AVGCORR2 = AVGCORR2;
    end
   ...
   %Calculate average AFR correction from target AFR vs. WBo2
        ...
        AFRCORR2 = AFRWBB4(i)/TARGETAFR1(i);
        end
    if AFRCORR2 > (1+MAXCORR);
        AFRCORR2 = (1+MAXCORR);
    else
        AFRCORR2 = AFRCORR2;
    end
    if AFRCORR2 < (1-MAXCORR);
        ...
        if AFCORRB4(i)==100 %use correction factors if closed loop, AFR values if open loop
            B2CORR(i) = AFRCORR2;
        else
            B2CORR(i) = AVGCORR2;
        end
...
for i=1:m;
    ...
    hits(i,1)=MAF(i);
    for j=1:n
    if (MAF(i)-0.04)<MASAFB1V1(j); 
        if MASAFB1V1(j)<(MAF(i)+0.04);
            MAFCORR1(i,2)=MAFCORR1(i,2)+B1CORR(j);
            hits(i,2)=hits(i,2)+1;
        else
            MAFCORR1(i,2)=MAFCORR1(i,2);
        hits(i,2)=hits(i,2);
        end
    else
        MAFCORR1(i,2)=MAFCORR1(i,2);
...
%%Average out MAF data
for i = 1:m;
    if hits(i,2)==0;
        
MAFCORR1(i,2)=MAFCORR1(i,2);
    else
        MAFCORR1(i,2)=MAFCORR1(i,2)/hits(i,2);
    end
    if MAFCORR1(i,2)==0;
        MAFCORR1(i,2)=1;
    else
...
%% Loop through bank 2 MAF sensor data
m = length(MAF);
hits = zeros(m,2);
MAFCORR2 = zeros(m,2);
...
for i=1:m;
    ...
    hits(i,1)=MAF(i);
    for j=1:n
    if (MAF(i)-0.04)<MASAFB2V1(j); 
        if MASAFB2V1(j)<(MAF(i)+0.04);
            MAFCORR2(i,2)=MAFCORR2(i,2)+B2CORR(j);
            hits(i,2)=hits(i,2)+1;
        else
            MAFCORR2(i,2)=MAFCORR2(i,2);
        hits(i,2)=hits(i,2);
        end
    else
        MAFCORR2(i,2)=MAFCORR2(i,2);
...
MAFCORR2(i,2)=MAFCORR2(i,2);
        ...
        MAFCORR2(i,2)=MAFCORR2(i,2)/hits(i,2);
    end
    if MAFCORR2(i,2)==0;
        MAFCORR2(i,2)=1;
    else
...
for i = 2:x;
    for j = 2:y;
        for k = 1:n;
            if (BFS(i-1)-0.2)<BFUELSCHDLms1(k);
                if BFUELSCHDLms1(k)< (BFS(i-1)+0.2);
...
for i = 2:x;
    for j = 2:y;
        for k = 1:n;
            if (BFS(i-1)-0.15)<BFUELSCHDLms1(k);
                if BFUELSCHDLms1(k)< (BFS(i-1)+0.15);
                    if (rpm(j-1)-150)<EngineSpeed1(k);
                        if EngineSpeed1(k)<(rpm(j-1)+150);
        ...
        for k = 1:n;
            if (BFS(i-1)-0.2)<BFUELSCHDLms1(k);
                if BFUELSCHDLms1(k)< (BFS(i-1)+0.2);
                    if (rpm(j-1)-200)<EngineSpeed1(k);
                        if EngineSpeed1(k)<(rpm(j-1)+200);
                            if INTAKEAIRTMP1(k) > MAXIAT(i,j);
                            MAXIAT(i,j)=INTAKEAIRTMP1(k);
                            else
                                MAXIAT(i,j)=MAXIAT(i,j);
                            end
...
%% loop through AFR data for bank 1
AFRbank1 = zeros(y+1,x+1);
fuelhits = zeros(y+1,x+1);

for i=1:y;
    AFRbank1(i+1,1)=rpm(i);
end
for i=1:x;
    AFRbank1(1,i+1)=fcomp(i);
end
        ...
        for k = 1:n;
            if (fcomp(i-1)-5)<FuelCompensationXTrace1(k);
                if FuelCompensationXTrace1(k)< (fcomp(i-1)+5);
                    if (rpm(j-1)-150)<EngineSpeed1(k);
                        if EngineSpeed1(k)<(rpm(j-1)+150);
                       if TARGETAFR1==30; %check to see if in decel
                                AFRbank1(i,j)=AFRbank1(i,j);
                            else
                                AFRbank1(i,j)=AFRbank1(i,j)+AFRWBB3(k);
                                fuelhits(i,j)=fuelhits(i,j)+1;
                            end
                        else
                        end
...
for i = 2:x+1;
    for j = 2:y+1;
        ...
        AFRbank1(i,j)=round(AFRbank1(i,j)/fuelhits(i,j),1);
    end
end

%% loop through AFR data for bank 2
AFRbank2 = zeros(y+1,x+1);
fuelhits = zeros(y+1,x+1);

for i=1:y;
    AFRbank2(i+1,1)=rpm(i);
end
for i=1:x;
    AFRbank2(1,i+1)=fcomp(i);
end
            ...
            if (fcomp(i-1)-5)<FuelCompensationXTrace1(k);
                if FuelCompensationXTrace1(k)< (fcomp(i-1)+5);
                    if (rpm(j-1)-150)<EngineSpeed1(k);                      
                        if EngineSpeed1(k)<(rpm(j-1)+150);
                            if TARGETAFR1==30; %check to see if in decel
                                AFRbank2(i,j)=AFRbank2(i,j);
                            else
                                AFRbank2(i,j)=AFRbank2(i,j)+AFRWBB4(k);
                                fuelhits(i,j)=fuelhits(i,j)+1;
                            end
                        else
...
for i = 2:x+1;
    for j = 2:y+1;
        ...
        AFRbank2(i,j)=round(AFRbank2(i,j)/fuelhits(i,j),1);
    end
end